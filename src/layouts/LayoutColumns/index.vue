...
<template>
  <el-container class="layout">
      ...
      <el-scrollbar>
        <div class="split-list">
          <div class="split-item"
            ...
            v-for="item in menuList" :key="item.path" @click="changeSubMenu(item)">
            <el-icon>
    ...
    <el-aside :class="{ 'not-aside': !subMenu.length }" :style="{ width: isCollapse ? '65px' : '210px' }">
      <div class="logo flx-center">
        <span v-show="subMenu.length">{{ isCollapse ? "D" : "DaoXiangLi" }}</span>
      </div>
      <el-scrollbar>
        <el-menu :default-active="activeMenu" :router="false" :collapse="isCollapse" :collapse-transition="false"
          :unique-opened="true" background-color="#ffffff">
